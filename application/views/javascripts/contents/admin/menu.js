$(document).ready(function () {
    ...
    $('#OkCheck').click(() => {
        ...
        window.apiClient.frontMenu.delete(id)
            .done((data) => {
                Toast.fire({
                    icon: 'success',
    ...
    const editRow = (id, data) => {
        ...
        $table.cell(row, 4).data(data.url)
        $table.cell(row, 5).data(data.status)
    }
    ...
    $('#form-tambah').submit((ev) => {
        ...
        if (id == "") {
            // Insert
            $.LoadingOverlay("show");
            window.apiClient.frontMenu.insert(menu_menu_id, nama, index, url, keterangan, status)
                .done((data) => {
                    Toast.fire({
                        icon: 'success',
            ...
            // Update
            $.LoadingOverlay("show");
            window.apiClient.frontMenu.update(id, menu_menu_id, nama, index, url, keterangan, status)
                .done((data) => {
                    Toast.fire({
                        icon: 'success',
...
const Ubah = (id) => {
    // $.LoadingOverlay("show");
    window.apiClient.frontMenu.detail(id)
        .done((data) => {
