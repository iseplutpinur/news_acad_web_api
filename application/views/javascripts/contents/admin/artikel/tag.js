$(function () {
  function dynamic() {
    ...
    const new_table = table_html.DataTable({
      "ajax": {
        "url": "<?= base_url()?>admin/artikel/tag/ajax_data/",
        "data": null,
        "type": 'POST'
      },
      "processing": true,
  ...
  $("#btn-tambah").click(() => {
    $("#tambahModalTitle").text("Tambah Tag");
    $('#id').val('');
    $('#nama').val('');
    $('#slug').val('');
    $('#foto').val('');
  ...
  $("#fmain").submit(function (ev) {
    ...
    $.ajax({
      method: 'post',
      url: '<?= base_url() ?>admin/artikel/tag/' + ($("#id").val() == "" ? 'insert' : 'update'),
      data: form,
      cache: false,
      contentType: false,
      processData: false,
  ...
  $('#OkCheck').click(() => {
    ...
    $.ajax({
      method: 'post',
      url: '<?= base_url() ?>admin/artikel/tag/delete',
      data: {
        id: id
      }
    }).done((data) => {
...
const view_gambar = (datas) => {
  $("#img-view").attr('src', `<?= base_url() ?>/files/artikel/tag/${datas.dataset.data}`)
}
...
const Ubah = (datas) => {
  ...
  $("#tambahModalTitle").text("Ubah Tag");
}